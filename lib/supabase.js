...
export const getAdminDashboardStats = async () => {
  try {
    const [issuesResult, postsResult, usersResult, tendersResult, feedbackResult, areasResult, departmentsResult] = await Promise.all([
      ...
      supabase.from('areas').select('id, name, is_active'),
      supabase.from('departments').select('id, name, category, is_active')
    ]);
    ...
    const stats = {
      ...
      active_areas: areas.filter(a => a.is_active).length,
...
// Multi-level admin functions

// Get areas for area super admin assignment
export const getAreas = async () => {
  try {
    const { data, error } = await supabase
      .from('areas')
...
// Get departments for department admin assignment
...
export const getAreaSuperAdminDashboard = async () => {
  try {
    ...
    if (!profile?.assigned_area_id) {
      throw new Error('No area assigned to this super admin');
    }

    // Get issues in this area
...
export const getDepartmentAdminDashboard = async () => {
  try {
    ...
    if (!profile?.assigned_department_id) {
      throw new Error('No department assigned to this admin');
    }
    ...
    const { data: contractors } = await supabase
      .from('profiles')
      .select('id, full_name, email, phone, specializations')
      .eq('user_type', 'tender')
      .eq('is_verified', true);
...
export const getContractorDashboard = async () => {
  try {
    ...
    // Get work progress
    const { data: workProgress, error: progressError } = await supabase
      .from('work_progress')
      .select('*')
      .eq('contractor_id', user.id)
      .order('created_at', { ascending: false });
    ...
    // Get available tenders for bidding
...
export const assignIssueToDepart = async (issueId, departmentId, assignmentNotes = '') => {
  try {
    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) {
      throw new Error('User not authenticated');
    ...
    const { data: department } = await supabase
      .from('departments')
      .select('head_admin_id')
      .eq('id', departmentId)
      .single();

    if (!department?.head_admin_id) {
      throw new Error('Department has no assigned admin');
    }
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        current_assignee_id: department.head_admin_id,
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        issue_id: issueId,
        assigned_by: user.id,
        assigned_to: department.head_admin_id,
...
export const assignTenderToContractor = async (tenderId, contractorId, contractAmount, terms = '') => {
  try {
    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) {
      throw new Error('User not authenticated');
...
// Final resolution by area super admin
export const markIssueResolvedBySuper = async (issueId, resolutionNotes) => {
  try {
    const { data: { user }, error: userError } = await supabase.auth.getUser();
    if (userError || !user) {
      throw new Error('User not authenticated');
    ...
    const { data, error } = await supabase
      ...
      .update({
        ...
        final_resolution_notes: resolutionNotes,
        resolved_by_super_admin: user.id,
...
// Get issues by workflow stage for different admin types
...
export const updateUserRole = async (userId, newRole, areaId = null, departmentId = null) => {
  try {
    const updates = {
      user_type: newRole,
      updated_at: new Date().toISOString()
    ...
    if (newRole === 'area_super_admin' && areaId) {
      updates.assigned_area_id = areaId;
      
      // Update area's super admin
      await supabase
        .from('areas')
        .update({ super_admin_id: userId })
        .eq('id', areaId);
    }

    if (newRole === 'department_admin' && departmentId) {
      updates.assigned_department_id = departmentId;
      
      // Update department's head admin
      await supabase
        .from('departments')
        .update({ head_admin_id: userId })
        .eq('id', departmentId);
    }
